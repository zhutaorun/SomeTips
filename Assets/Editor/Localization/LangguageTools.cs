...
public class LangguageTools
{
    private static string simpPYStr()
    {
    ...
    private static string s_simpStr;
    private static string s_ftStr;
    private static Dictionary<string, string> s_zh_cn2tw_map;
    public static string To_zh_tc(string text)
    {
        if(s_simpStr == null)
            s_simpStr = simpPYStr();
        if(s_ftStr == null)
            s_ftStr = ftPYStr();
        if (s_zh_cn2tw_map == null)
            s_zh_cn2tw_map = getMap();
        StringBuilder sb = new StringBuilder(text);
        //优先替换词语
        foreach (var key in s_zh_cn2tw_map.Keys)
        {
            replaceWord(sb, key, s_zh_cn2tw_map[key]);
        }
        for(var i=0;i<sb.Length;i++)
        {
            int idx = s_simpStr.IndexOf(sb[i]);
            if(idx != -1)
            {
                sb[i] = s_ftStr[idx];
            }
    ...
    public static bool Hac_zh_tc(string text)
    {
        if (s_simpStr == null)
            s_simpStr = simpPYStr();
        if (s_ftStr == null)
    ...
    static void replaceWord(StringBuilder sb,  string key, string value)
    {
        int idx = -1;
        string src = null;
        do{
